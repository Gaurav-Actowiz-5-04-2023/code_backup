...
OUTPUT_LOCATION = "D:/Data/gaurav/output_file/OCTOBER/"
...
file_name = output_folder + f"full_{TODAY_DATE}-v1.json"
...
def junk_changes(attributes):
    if attributes:
        ...
        try:
            return [i for i in attr_list if i['name'] != 'nan']
        except Exception as e:
            return attr_list
    else:
        return attributes
...
for i in ['attributes', 'features']:
    product_df[i] = product_df[i].apply(junk_changes)


# junk character remove in all columns :-
def junk_change(attributes):
    if isinstance(attributes, dict) and attributes and str(attributes) != 'nan':
        attr_dict = dict()
        for key in attributes:
            attr_dict[key] = html.unescape(attributes[key]) if attributes[key] else None
        return attr_dict
        ...
        try:
            for dictionary in attributes:
                attr_dict = dict()
                for key in dictionary:
                    attr_dict[key] = html.unescape(dictionary[key]) if dictionary[key] else None
                attr_list.append(attr_dict)
            return attr_list
        except:
            return attributes
...
for i in ['main_image', 'assets']:
    product_df[i] = product_df[i].apply(junk_change)
...
product_df['mpn'] = product_df['sku']

product_df['name'] = product_df['name'].apply(lambda x: x.replace('&nbsp', '') if x else x)
