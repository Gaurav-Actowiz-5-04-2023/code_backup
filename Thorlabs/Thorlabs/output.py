...
from validation_function import data_validations
...
asset_df = asset_df.drop_duplicates(subset=['source', 'sku'])
...
product_df = pd.read_sql(
    sql=(
        ...
        f"from {product_table} where vendor_id = '{VENDOR_ID}' and status='Done' ;"
    ),
    con=con
)
...
def junk_remove(attributes):
    for attribute in attributes:
        for key in attribute:
            attribute[key] = html.unescape(attribute[key].replace('\\xa0', ' '))
    return attributes


product_df['attributes'] = product_df['attributes'].apply(junk_remove)
...
validation = data_validations(product_df)
